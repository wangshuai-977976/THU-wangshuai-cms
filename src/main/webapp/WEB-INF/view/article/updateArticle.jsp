...
<body>
		...
		<div class="form-group row ">
			<img alt="" style="widows: 80px;height: 80px" src="/pic/${article.picture }">
		...
		<div class="form-group row ">
			...
			<select class="custom-select custom-select-sm mb-3" id="channel"  name="channelId">
			  ...
			  <c:forEach items="${channelList}" var="channel">
			  		<option value="${channel.id}" ${article.channel.id == channel.id ? 'selected' : '' }>${channel.name}</option>
			  </c:forEach>
			</select>
				...
				success:function(data){
					if (data.status == 100) {
						for ( var i in data.info.categorys) {
							if (data.info.categorys[i].id == "${article.category.id}") {
								$("#category").append("<option selected value='"+data.info.categorys[i].id+"'>"+data.info.categorys[i].name+"</option>");
							}else{
